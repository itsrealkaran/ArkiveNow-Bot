  ...
  async getMentions(lastCheckedTime?: string): Promise<ScrapedMention[]> {
    ...
    try {
      ...
      try {
        ...
        console.log('Navigated to mentions page');
        await new Promise(resolve => setTimeout(resolve, 5000));
        // Scroll to top and reload for assurance
        ...
        // If there are no tweets newer than lastCheckedTime, try switching to 'All' tab and back to 'Mentions' tab until such tweets are found or max attempts
        let hasNewTweet = false;
        let attempts = 0;
        const maxAttempts = 5;
        while (attempts < maxAttempts) {
          hasNewTweet = await this.page.evaluate((lastCheckedTime) => {
            const articles = Array.from(document.querySelectorAll('article[data-testid="tweet"]'));
            for (const article of articles) {
              const timeElement = article.querySelector('time');
              const timestamp = timeElement?.getAttribute('datetime') || '';
          ...
          console.log('Has new tweet after lastCheckedTime:', hasNewTweet);
          if (hasNewTweet) break;
          // Click on 'All' tab
          const allTab = await this.page.$('a[role="tab"][href*="/notifications"]');
          if (allTab) {
            await allTab.click();
            await new Promise(resolve => setTimeout(resolve, 2000));
          }
          // Click back to 'Mentions' tab
          const mentionsTab = await this.page.$('a[role="tab"][href*="/notifications/mentions"]');
          if (mentionsTab) {
